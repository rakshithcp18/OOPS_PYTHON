class VendingMachine:
    ...
    snack_prices = {"candy": 2.00, "soda": 1.50, "chips": 3.00, "cookies": 3.50}   # dictionary with {<snack>: <amount>} as key-value pairs. Possible snacks: candy, soda, chips, cookies. Keys written in lowercase.
 
    # Instance attributes
    def __init__(self, inventory, serial, days_until_maintenance):           # Initially, when an instance of the vending machine is created, the revenue is 0 and it's updated with each sale.
    ...
    def sales_menu(self):
        ...
        while True:
            ...
            # Repeat if the input doesn't meet the requireme1nts
    ...
    def process_sale(self, option): # option must be in lowercase
        ...
        if self.inventory[option] > 0:
            ...
            if num_items <= self.inventory[option]:
                ...
                # Remove item from menu if inventory is depleted
                if self.inventory[option] == 0:
...
class HospitalVendingMachine(VendingMachine):
    ...
    snack_prices = {"candy": 1.00, "soda": 15.00, "chips": 30.0, "cookies": 35.0}
    ...
    def print_days_until_maintenance(self):
        print("i have %d until my next days of the maintance " % self.days_until_maintenance)



class SchoolVendingMachine(VendingMachine):

    snack_prices = {"candy": 0.50, "soda": 1.00, "chips": 3.0, "cookies": 3.50}

    student_debt ={"abc":50 , "def":60}
    ...
    def print_student_debt(self,student):
        print(" the student %s has having reaming debt of $%d " % (student,SchoolVendingMachine.student_debt))
...
# school_machine = SchoolVendingMachine({"candy": 36, "soda": 15, "chips": 40, "cookies": 120}, "0534424", 12) 
